...
double evaluate_by_summing_basis(const ModalVector& coefficients,
                                 const double x) {
  double sum = 0.0;
  for (size_t k = 0; k < coefficients.size(); ++k) {
    sum += coefficients[k] *
           Spectral::compute_basis_function_value<Spectral::Basis::Chebyshev>(
               k, x);
  }
  return sum;
}
...
void check_against_irregular(const std::array<size_t, Dim>& extents_array,
                             const std::array<double, Dim>& coords) {
  Mesh<Dim> mesh(extents_array, Spectral::Basis::Chebyshev,
  ...
  for (size_t i = 0; i < modal_coefficients.size(); ++i) {
    modal_coefficients[i] =
        0.3 - 0.02 * static_cast<double>(i % 7) + 0.1 * static_cast<double>(i);
  }
  ...
  CHECK(Spectral::evaluate_chebyshev_series<Dim>(
            modal_coefficients, mesh, logical_point) == approx(nodal_value[0]));
}

}  // namespace

SPECTRE_TEST_CASE("Unit.Numerical.Spectral.Chebyshev.Series",
                  "[NumericalAlgorithms][Spectral][Unit]") {
  const std::array<double, 5> evaluation_points{{-0.95, -0.31, 0.0, 0.42, 0.99}};

  ModalVector cubic_coefficients{4};
  cubic_coefficients[0] = 0.3;
  cubic_coefficients[1] = -1.1;
  cubic_coefficients[2] = 0.7;
  cubic_coefficients[3] = 2.5;
  Mesh<1> cubic_mesh({{cubic_coefficients.size()}}, Spectral::Basis::Chebyshev,
                     Spectral::Quadrature::GaussLobatto);
  for (const double x : evaluation_points) {
    CHECK(Spectral::evaluate_chebyshev_series<1>(
              cubic_coefficients, cubic_mesh, make_logical_point<1>({{x}})) ==
          approx(evaluate_by_summing_basis(cubic_coefficients, x)));
  }
  ...
  higher_order[5] = 0.33;
  higher_order[6] = -2.1;
  Mesh<1> higher_mesh({{higher_order.size()}}, Spectral::Basis::Chebyshev,
                      Spectral::Quadrature::GaussLobatto);
  for (const double x : evaluation_points) {
    CHECK(Spectral::evaluate_chebyshev_series<1>(
              higher_order, higher_mesh, make_logical_point<1>({{x}})) ==
          approx(evaluate_by_summing_basis(higher_order, x)));
  }
