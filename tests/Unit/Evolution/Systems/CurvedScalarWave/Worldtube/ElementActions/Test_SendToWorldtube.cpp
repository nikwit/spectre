      ...
      Parallel::PhaseActions<
          ...
          tmpl::list<ActionTesting::InitializeDataBox<
              db::AddSimpleTags<
                  ...
                  gr::Tags::Shift<DataVector, Dim>, gr::Tags::Lapse<DataVector>,
                  ::Tags::TimeStepId, Tags::CurrentIteration>,
              db::AddComputeTags<>>>>,
...
// check that it is unused on this code path. The worldtube sphere is static
// and centered at the origin, so the centered inertial face coordinates
// coincide with the grid coordinates of the worldtube sphere and their
// radius equals the grid excision radius as required by this code path.
void test_s2_projection() {
  ...
  for (const auto& [expansion_order, worldtube_radius] : cartesian_product(
    ...
    // the particle is at the center of the excision sphere and at rest, so
    // the centered inertial face coordinates are just the grid coordinates
    // of the worldtube sphere
    tnsr::I<DataVector, Dim, Frame::Inertial> face_coords(face_size);
    ...
    ActionTesting::emplace_singleton_component_and_initialize<worldtube_chare>(
        ...
        {std::unordered_map<ElementId<Dim>,
         ...
         particle_pos_vel, background_quantities, evolved_vars_wt,
         dt_variables});
...
// This test checks that `SendToWorldtube` projects the regular field and its
// inertial-frame time derivative onto real spherical harmonics evaluated at
// the centered inertial coordinates of the element surfaces abutting the
// worldtube and sends this data to the worldtube which reduces it in
// `ReceiveElementData`. There are several other actions in the action lists
// which are needed for the test to compile but are never used. The iterative
// scheme is tested in Test_Iterations.cpp.
      ...
      if (expansion_order > 1) {
        for (size_t i = 0; i < Dim; ++i) {
          for (size_t j = 0; j < Dim; ++j) {
            get(get<CurvedScalarWave::Tags::Psi>(evolved_vars)) +=
                psi_coefs_2.get(i, j) * inertial_coords.get(i) *
                inertial_coords.get(j);
            get(get<CurvedScalarWave::Tags::Pi>(evolved_vars)) +=
                pi_coefs_2.get(i, j) * inertial_coords.get(i) *
                inertial_coords.get(j);
          }
        }
      }
        ...
        const auto& dt_psi_quadrupole_worldtube =
            ActionTesting::get_databox_tag<
                ...
                Stf::Tags::StfTensor<::Tags::dt<Tags::PsiWorldtube>, 2, Dim,
                                     Frame::Inertial>>(runner, 0);
        tnsr::ii<double, Dim, Frame::Inertial> expected_psi_quadrupole{};
        tnsr::ii<double, Dim, Frame::Inertial> expected_dt_psi_quadrupole{};
        for (size_t i = 0; i < Dim; ++i) {
          for (size_t j = 0; j < Dim; ++j) {
