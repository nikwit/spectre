...
double evaluate_legendre_segment(gsl::span<const double> coefficients,
  ...
  double P_k_minus_two = 1.0;
  double sum = gsl::at(coefficients, 0) * P_k_minus_two;
  if (coefficients.size() == 1) {
    return sum;
  }
  double P_k_minus_one = x;
  sum += gsl::at(coefficients, 1) * P_k_minus_one;
  for (size_t k = 2; k < coefficients.size(); ++k) {
    const double P_k =
        ((2.0 * k - 1.0) * x * P_k_minus_one - (k - 1.0) * P_k_minus_two) / k;
    sum += gsl::at(coefficients, k) * P_k;
    P_k_minus_two = P_k_minus_one;
    P_k_minus_one = P_k;
  }
  return sum;
}
