...
void apply_tensor_ylm_filter(
  ...
  tmpl::for_each<filter_detail::gh_spatial_vars_list<Frame::Grid>>(
      [&gh_spatial_spectral_vars, &temp_spatial_vars, radial_extents,
       ...
       &filter_matrix_kii, &filter_matrix_scalar,
       &ylm]<class Tag>(const tmpl::type_<Tag> /*meta*/) {
        ...
        ASSERT(
            ...
            "Insufficient size: must have "
                ...
                << num_independent_components << ", l_max = " << ylm.l_max());
...
void apply_tensor_ylm_filter(
  ...
  const size_t spectral_points = radial_extents * ylm.spectral_size();
  ASSERT(physical_points == sw_vars->number_of_grid_points(),
         "Mismatch " << physical_points << " must equal "
                     << sw_vars->number_of_grid_points());

  // Allocate all temporary storage locally to avoid shared memory.
  Variables<filter_detail::sw_vars_list<Frame::Grid>> grid_vars(
      physical_points);
  Variables<filter_detail::sw_vars_list<Frame::Grid>> sw_spectral_vars(
      spectral_points);
  Variables<filter_detail::sw_vars_list<Frame::Grid>> filtered_grid_vars(
      physical_points);
  ...
  // dest: temp_grid_vars
  filter_detail::transform_spatial_tensors_to_different_frame_without_hessians<
      Frame::Inertial, Frame::Grid>(make_not_null(&grid_vars), *sw_vars,
                                    jac_inertial_to_grid);
  /*Parallel::printf(
      "CSW TensorYlmFilter debug: Phi nodal (grid frame) before filter "
      "matrix apply = %s\n",
      get<CurvedScalarWave::Tags::Phi<3, Frame::Grid>>(grid_vars));*/
  ...
  // dest: temp_grid_vars
  filter_detail::modal_to_nodal_ylm(make_not_null(&filtered_grid_vars),
                                    sw_spectral_vars, ylm, radial_extents);
  /*Parallel::printf(
      "CSW TensorYlmFilter debug: Phi nodal (grid frame) after filter matrix "
      "apply = %s\n",
      get<CurvedScalarWave::Tags::Phi<3, Frame::Grid>>(filtered_grid_vars));*/

  // 5. Multiply by Jacobians to get back into inertial frame.
  // src: temp_grid_vars
  // dest: sw_vars
  filter_detail::transform_spatial_tensors_to_different_frame_without_hessians<
      Frame::Grid, Frame::Inertial>(sw_vars, filtered_grid_vars,
                                    jac_grid_to_inertial);
}
...
CurvedScalarWaveTensorYlmFilter::CurvedScalarWaveTensorYlmFilter() = default;

CurvedScalarWaveTensorYlmFilter::CurvedScalarWaveTensorYlmFilter(
    const CurvedScalarWaveTensorYlmFilter& rhs)
    : num_modes_to_kill_(rhs.num_modes_to_kill_),
      half_power_(rhs.half_power_),
      enable_(rhs.enable_) {}

CurvedScalarWaveTensorYlmFilter& CurvedScalarWaveTensorYlmFilter::operator=(
    const CurvedScalarWaveTensorYlmFilter& rhs) {
  if (this != &rhs) {
    num_modes_to_kill_ = rhs.num_modes_to_kill_;
    half_power_ = rhs.half_power_;
...
void CurvedScalarWaveTensorYlmFilter::pup(PUP::er& p) {
  p | num_modes_to_kill_;
  p | half_power_;
  p | enable_;
  // The filter matrices and temp storage are lazily initialized,
...
void CurvedScalarWaveTensorYlmFilter::operator()(
    const gsl::not_null<
        Variables<filter_detail::sw_vars_list<Frame::Inertial>>*>
        sw_vars,
    const Mesh<2>& mesh,
  ...
  ASSERT(mesh.basis(1) == Spectral::Basis::SphericalHarmonic,
         "CurvedScalarWaveTensorYlmFilter requires spherical harmonic basis "
         "in both angular directions for Dim=2.");
  const size_t radial_extents = 1;
  const size_t l_max = mesh.extents(0) - 1;
