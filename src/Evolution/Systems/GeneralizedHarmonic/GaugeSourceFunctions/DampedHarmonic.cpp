...
void damped_harmonic_impl(
  ...
  get(prefac) = get(mu_L1) * get(log_fac_1);
  get(prefac) += get(mu_L2) * get(log_fac_2);
  ...
  // We only need \f$\partial_a \ln(\det\gamma) = \gamma^{jk}\partial_a
  // \gamma_{jk}\f$ (misleadingly named `d_g_by_det`). Compute it directly from
  // the spatial-spatial block of the spacetime metric derivative
  ...
  const auto spacetime_deriv_of_power_log_factor_metric_lapse =
      [&d_lapse_by_lapse, &d_g_by_det, &prefac_log, &d_logfac](
  ...
  if constexpr (UseRollon) {
    ...
    for (size_t a = 0; a < SpatialDim + 1; ++a) {
      // \f$ \partial_a \mu_1 \f$
      d4_mu1.get(a) = get(pow1) * d4_weight.get(a) +
                      get(weighted_amp_L1) * d4_log_fac_mu1.get(a);
      // \f$ \partial_a \mu_{S} \f$
      d4_mu_S.get(a) = d4_weight.get(a) * get(pow2) +
                       get(weighted_amp_S) * d4_log_fac_muS.get(a);
      // \f$ \partial_a \mu_2 \f$
      d4_mu2.get(a) = get(pow3) * d4_weight.get(a) +
                      get(weighted_amp_L2) * d4_log_fac_mu2.get(a);
    }
  }
