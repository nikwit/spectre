...
namespace gh::Batched::Initialization {

struct DriverState {
  using packed_system = gh::System<3>;
  ...
  using simple_tags =
      tmpl::list<packed_topology_tag, packed_geometry_tag,
                 ...
                 packed_boundary_scratch_tag, device_constraint_gamma0_tag,
  ...
  static void apply(
    ...
    for (const auto& block : domain.blocks()) {
      ...
      if (not have_first_extents) {
        ...
        ASSERT(block_extents == first_extents,
               "EvolveGhNoBlackHoleKokkosBatched currently requires uniform "
               "extents across all blocks/elements.");
      }
    ...
    packed_topology->element_extents_device = ::Kokkos::View<size_t* [3]>(
        "GhBatchedElementExtents",
        packed_topology->element_extents_host.size());
    auto host_extents =
    ...
    for (size_t d = 0; d < volume_dim; ++d) {
      ...
      lower_face_to_volume_index = ::Kokkos::View<size_t*>(
          "GhBatchedLowerFaceToVolumeIndexMap", num_face_points);
      upper_face_to_volume_index = ::Kokkos::View<size_t*>(
          "GhBatchedUpperFaceToVolumeIndexMap", num_face_points);
