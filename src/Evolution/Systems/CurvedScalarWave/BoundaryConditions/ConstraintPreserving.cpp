...
ConstraintPreserving<Dim>::get_clone() const noexcept {
  return std::make_unique<ConstraintPreserving>(*this);
}

template <size_t Dim>
void ConstraintPreserving<Dim>::pup(PUP::er& p) {
  BoundaryCondition<Dim>::pup(p);
}

template <size_t Dim>
ConstraintPreserving<Dim>::ConstraintPreserving(
    CkMigrateMessage* const msg) noexcept
    : BoundaryCondition<Dim>(msg) {}

template <size_t Dim>
std::optional<std::string> ConstraintPreserving<Dim>::dg_time_derivative(
  ...
  const auto evolved_fields = evolved_fields_from_characteristic_fields(
  ...
  *dt_pi_correction = get<Pi>(evolved_fields);
  *dt_psi_correction = get<Psi>(evolved_fields);
  *dt_phi_correction = get<Phi<Dim>>(evolved_fields);
...
template <size_t Dim>
// NOLINTNEXTLINE
PUP::able::PUP_ID ConstraintPreserving<Dim>::my_PUP_ID = 0;

#define DIM(data) BOOST_PP_TUPLE_ELEM(0, data)

#define INSTANTIATION(r, data) template class ConstraintPreserving<DIM(data)>;
