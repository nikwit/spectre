...
struct SendAccelerationTerms {
  ...
  using simple_tags = tmpl::list<Tags::RegularFieldAdvectiveTerm<Dim>,
                                 Tags::PunctureFieldAccelerated<Dim>>;
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    if (puncture_field.has_value()) {
      ...
      const auto self_force = get(inbox.at(time_step_id));
      inbox.erase(time_step_id);

      db::mutate<Tags::PunctureFieldAccelerated<Dim>>(
          make_not_null(&box),
          [&self_force,
           &position_velocity =
      ...
      psi_regular_times_det =
          get(face_psi) -
          get(get<CurvedScalarWave::Tags::Psi>(acc_puncture_field));
      ...
      dt_psi_regular_times_det =
          ...
          get(get<::Tags::dt<CurvedScalarWave::Tags::Psi>>(acc_puncture_field));
