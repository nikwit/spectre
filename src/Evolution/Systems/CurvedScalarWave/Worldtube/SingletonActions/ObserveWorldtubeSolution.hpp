...
struct ObserveWorldtubeSolution {
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    if (db::get<Tags::ObserveCoefficientsTrigger>(box).is_triggered(box) and
      ...
      // at second order the constant coefficient is evolved separately
      // because the monopole on the worldtube boundary also contains a
      // contribution of the trace of the second-order coefficient
      ...
      if (expansion_order > 1) {
        ...
        ASSERT(offset == num_coefs + pos_offset, "Internal indexing error");
      }
