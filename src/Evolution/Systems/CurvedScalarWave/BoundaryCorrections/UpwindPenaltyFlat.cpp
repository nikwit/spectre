...
template <size_t Dim>
UpwindPenaltyFlat<Dim>::UpwindPenaltyFlat(CkMigrateMessage* msg) noexcept
    : BoundaryCorrection<Dim>(msg) {}

template <size_t Dim>
std::unique_ptr<BoundaryCorrection<Dim>> UpwindPenaltyFlat<Dim>::get_clone()
    const noexcept {
  return std::make_unique<UpwindPenaltyFlat>(*this);
}

template <size_t Dim>
void UpwindPenaltyFlat<Dim>::pup(PUP::er& p) {
  BoundaryCorrection<Dim>::pup(p);
}

template <size_t Dim>
double UpwindPenaltyFlat<Dim>::dg_package_data(
    ...
    const gsl::not_null<tnsr::i<DataVector, 4, Frame::Inertial>*>
  ...
  // interface.
  //
  // Note: when UpwindPenaltyFlat::dg_boundary_terms() is called, an Element
  // passes in its own packaged data to fill the interior fields, and its
  // neighbor's packaged data to fill the exterior fields. This introduces a
  // sign flip for each normal used in computing the exterior fields.
...
template <size_t Dim>
void UpwindPenaltyFlat<Dim>::dg_boundary_terms(
    ...
    const tnsr::i<DataVector, 4, Frame::Inertial>& char_speeds_int,
    ...
    const tnsr::i<DataVector, 4, Frame::Inertial>& char_speeds_ext,
...
template <size_t Dim>
// NOLINTNEXTLINE
PUP::able::PUP_ID UpwindPenaltyFlat<Dim>::my_PUP_ID = 0;

#define DIM(data) BOOST_PP_TUPLE_ELEM(0, data)

#define INSTANTIATION(_, data) template class UpwindPenaltyFlat<DIM(data)>;
