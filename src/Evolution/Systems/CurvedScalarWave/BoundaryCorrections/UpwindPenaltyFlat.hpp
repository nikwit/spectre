 ...
 */
template <size_t Dim>
class UpwindPenaltyFlat final : public BoundaryCorrection<Dim> {
 private:
  ...
  struct CharSpeedsTensor : db::SimpleTag {
    using type = tnsr::i<DataVector, 4, Frame::Inertial>;
  };

 public:
  using options = tmpl::list<>;
  static constexpr Options::String help = {
      "Computes the UpwindPenaltyFlat boundary correction term for the scalar "
      "wave "
      "system."};

  UpwindPenaltyFlat() = default;
  UpwindPenaltyFlat(const UpwindPenaltyFlat&) = default;
  UpwindPenaltyFlat& operator=(const UpwindPenaltyFlat&) = default;
  UpwindPenaltyFlat(UpwindPenaltyFlat&&) = default;
  UpwindPenaltyFlat& operator=(UpwindPenaltyFlat&&) = default;
  ~UpwindPenaltyFlat() override = default;

  /// \cond
  explicit UpwindPenaltyFlat(CkMigrateMessage* msg) noexcept;
  using PUP::able::register_constructor;
  WRAPPED_PUPable_decl_template(UpwindPenaltyFlat);  // NOLINT
  /// \endcond
  ...
  double dg_package_data(
      ...
      gsl::not_null<tnsr::i<DataVector, 4, Frame::Inertial>*>
