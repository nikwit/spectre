...
bool receive_boundary_data_global_time_stepping_kokkos(
  ...
  auto& inbox = tuples::get<
      ScalarWave::KokkosTags::BoundaryCorrectionInbox<3, false>>(*inboxes);
...
struct ApplyBoundaryCorrectionsToTimeDerivativeKokkos {
 private:
  ...
  using device_dt_variables_tag = ScalarWave::KokkosTags::DeviceDtVariables<System>;
 ...
 public:
  ...
  using inbox_tags =
      tmpl::list<ScalarWave::KokkosTags::BoundaryCorrectionInbox<
          3, false>>;
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    ASSERT(
        Parallel::number_of_nodes<size_t>(cache) == 1,
    ...
    db::mutate<device_dt_variables_tag>(
        [&outgoing_boundary_data =
         ...
         &element = db::get<domain::Tags::Element<3>>(box)](
            gsl::not_null<device_dt_type*> device_dt) {
          apply_boundary_corrections_on_device(
              device_dt, outgoing_boundary_data, incoming_boundary_data,
              device_face_to_volume_index_map, device_face_normal_magnitude,
              mesh, element);
        },
        make_not_null(&box));
