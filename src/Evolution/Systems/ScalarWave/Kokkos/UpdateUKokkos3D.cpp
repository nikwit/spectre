...
void UpdateUKokkos3D::apply(
  ...
  ASSERT(substep < number_of_substeps,
         "Substep should be less than " << number_of_substeps << ", not "
                                        << substep);
  ...
  if (substep == number_of_substeps - 1) {
    ...
    coefficients = &runge_kutta->butcher_tableau().substep_coefficients[substep];
  }
