...
void TimeDependentMapOptions::build_maps(
  ...
  rotation_map_ = RotationMap3D{RotationMap2D{rotation_name}, IdentityMap1D{}};
...
TimeDependentMapOptions::distorted_to_inertial_map(
  ...
  if (include_distorted_map) {
    return std::make_unique<DistortedToInertialComposition>(
        expansion_map_interior_, rotation_map_);
  } else {
    return nullptr;
  }
}
...
TimeDependentMapOptions::grid_to_inertial_map(
    const bool include_distorted_map, const bool expansion_interior) const {
  if (include_distorted_map) {
    const size_t index = get_index(Object);
    return std::make_unique<GridToInertialComposition<true, true>>(
        gsl::at(size_maps_, index), expansion_map_interior_, rotation_map_);
  } else {
    if (expansion_interior) {
      return std::make_unique<GridToInertialComposition<false, true>>(
          expansion_map_interior_, rotation_map_);
    } else {
      return std::make_unique<GridToInertialComposition<false, false>>(
          expansion_map_exterior_, rotation_map_);
    }
  }
}
